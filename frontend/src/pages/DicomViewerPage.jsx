...
useEffect(() => {
  const initViewer = async () => {
    ...
    const planes = [
      { id: 'axial', ref: axialRef },
      { id: 'sagittal', ref: sagittalRef },
      { id: 'coronal', ref: coronalRef }
    ];

    const imageIds = ['wadouri:/Dicom/1.2.826.0.1.3680043.8.498.65452424240994805812717428674475343109-c.dcm'];

    for (let plane of planes) {
      if (!plane.ref.current) continue;

      const viewportInput = {
        viewportId: plane.id,
        type: 'stack',
        element: plane.ref.current,
        defaultOptions: { background: [0, 0, 0] }
      };

      renderingEngine.enableElement(viewportInput);
      const viewport = renderingEngine.getViewport(plane.id);
 ...
 <div ref={plane === 'axial' ? axialRef : plane === 'sagittal' ? sagittalRef : coronalRef} 
